...
class PatchDir:
	...
	def __str__(self) -> str:
		return f"<PatchDir: full_dir:'{str(self.full_dir)}'>"
	...
	def find_series_patch_files(self) -> list["PatchFileInDir"]:
		...
		if os.path.isfile(series_conf_path):
			...
			for counter, patch_file_name in enumerate(patches_in_series, start=1):
	...
	def split_patches_from_file(self) -> list["PatchInPatchFile"]:
		...
		if self.patch_dir.is_autogen_dir:
			...
			bare_patch = PatchInPatchFile(
				...
				"",
				"Autogenerated patch",
				"Armbian Autopatcher <auto.patch@armbian.com>",
				f"[AUTOGEN] {self.relative_dirs_and_base_file_name}",
				None,
			)
		...
		if not patches:
			raise Exception(f"No valid patches found in file {self.full_file_path()}")
		return patches
...
class PatchInPatchFile:
	...
	def parse_from_name_email(self, from_str: str) -> tuple["str | None", "str | None"]:
		...
		if m is not None:
			...
			return downgrade_to_ascii(remove_quotes(m["name"])), remove_quotes(m["email"])
	...
	def patch_rebase_tags_desc(self):
		tags = {
			"Patch-File": self.parent.relative_dirs_and_base_file_name,
			"Patch-File-Counter": self.counter,
			"Patch-Rel-Directory": self.parent.patch_dir.rel_dir,
			"Patch-Type": self.parent.patch_dir.patch_root_dir.patch_type,
			"Patch-Root-Type": self.parent.patch_dir.root_type,
			"Patch-Sub-Type": self.parent.patch_dir.sub_type,
		}
		if self.subject is not None:
			tags["Original-Subject"] = self.subject
		return "".join(f"X-Armbian: {k}: {v}\n" for k, v in tags.items())

	def markdown_applied(self):
		return "✅" if self.applied_ok else "❌"
	...
	def text_files(self):
		...
		if not file_names:
			return "?"
		ret = [f"{file_name}" for file_name in file_names[:max_files_shown]]
	...
	def text_author(self):
		return f"{self.from_name.strip()}" if self.from_name else "[no Author]"

	def markdown_author(self):
		return f"`{self.from_name.strip()}`" if self.from_name else "`[no Author]`"

	def markdown_subject(self):
		return f"_{self.subject}_" if self.subject else "`[no Subject]`"

	def text_subject(self):
		return f"{self.subject}" if self.subject else "[no Subject]"

	def markdown_link_to_patch(self):
		return "" if self.git_commit_hash is None else f"{self.git_commit_hash} "
	...
	def rich_name_status(self):
		color = "green"
		for problem in self.problems:
			color = "yellow" if problem in ["not_mbox", "needs_rebase"] else "red"
...
def perform_git_archeology(
	...
	other_suspects_desc: list[str] = [
		f"> recovered message: > {suspect_desc_line}"
		for suspect_desc_line in suspect_desc_lines
	]
	...
	if patch.subject is None:
		patch.subject = downgrade_to_ascii(f"[ARCHEOLOGY] {main_suspect_subject}")
