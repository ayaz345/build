...
reprepro_conf_distributions_fn = os.path.join(
	reprepro_conf_output_dir, "distributions"
)
reprepro_conf_options_fn = os.path.join(reprepro_conf_output_dir, "options")
reprepro_output_script_fn = os.path.join(
	reprepro_script_output_dir, "reprepro.sh"
)
...
# for each target
log.info("Generating repo config...")

all_distro_lines: list[str] = []
for one_repo_target in repo_targets:
	distro_dict: dict[str, str] = {
		"Origin": f"Armbian  origin {one_repo_target}",
		"Label": f"Armbian label {one_repo_target}",
		"Codename": f"{one_repo_target}",
		"Suite": f"{one_repo_target}",
		"Architectures": "amd64 armhf arm64 riscv64",
		"Components": "main",
		"Description": "Apt repository for Armbian",
	}
	if (gpg_keyid is not None) and (gpg_keyid != ""):
		...
		log.warning(
			"Didn't get REPO_GPG_KEYID from environment. Will not sign the repo."
		)

	all_distro_lines.extend(
		f"{key}: {value}" for key, value in distro_dict.items()
	)
...
for one_repo_target, artifacts in repo_targets.items():
	...
	if all_debs_to_include:
		...
		bash_lines.extend(
			(
				f"echo 'reprepro importing {len(all_debs_to_include_quoted)} debs for target {one_repo_target}...' ",
				" ".join(cmds),
			)
		)
# Always export at the end
export_cmds = ["reprepro", "-b", '"${REPO_LOCATION}"', "export"]
bash_lines.extend(("echo 'reprepro exporting...' ", " ".join(export_cmds)))
